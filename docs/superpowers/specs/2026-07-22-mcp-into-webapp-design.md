...
Sau khi xong: Claude kết nối thẳng `https://<production-domain>/mcp?key=<key>`; 14 tool gọi
...
- **Kết nối cũ đứt (không tránh được):** URL đổi `…workers.dev/mcp` → `<production-domain>/mcp`.
